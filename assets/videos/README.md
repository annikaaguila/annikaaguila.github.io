...
| Fashion Brand Reel | `nycgods-draft9.mp4`, `gods-nyc-reel-2.mp4` |
...
`gods-nyc-reel-2.mp4` started as a `.mov` export — Chrome/Firefox don't
support the QuickTime container even when the codecs inside (H.264/AAC)
are otherwise fine, so it was remuxed (no re-encode, same quality) to
`.mp4` with `avconvert --preset PresetPassthrough` since ffmpeg wasn't
available. If a future clip arrives as `.mov`, either re-run that or use
the ffmpeg recipe below with `-c copy` instead of re-encoding.
